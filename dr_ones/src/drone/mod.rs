...
impl Dr_One {
    ...
    fn broadcast_packet(&self, mut packet: Packet) {
        ...
        // Check if the packet is a FloodRequest
        if let PacketType::FloodRequest(ref mut flood_request) = packet.pack_type {
            // Add self to the path_trace
            flood_request.path_trace.push((self.id, NodeType::Drone));

            if self.seen_flood_ids.iter().any(|id| *id == flood_request.flood_id /* || it has no neighbors*/) {
                // TODO: create a flood response and send it back
            } else {
                // should broadcast only if we haven't already seen flood_requests with the same flood_id
                should_broadcast = true;
            }
        }

        // If the packet is a FloodRequest, perform the broadcast
        if should_broadcast {
            // TODO: Modify the following logic keeping in mind what the AP-protocol says 

            for (&node_id, sender) in self.packet_send.iter() {
                // Check if the node is not already in the path_trace
                if let PacketType::FloodRequest(ref flood_request) = packet.pack_type {
                    if !flood_request
                        .path_trace
                        .iter()
                        .any(|(id, _)| *id == node_id)
                    {
                        // Clone the packet for each neighbor
                        let cloned_packet = packet.clone();

                        // Send the cloned packet
                        if let Err(e) = sender.send(cloned_packet) {
                            println!("Failed to send packet to NodeId {:?}: {:?}", node_id, e);
                        }
                    }
                }
