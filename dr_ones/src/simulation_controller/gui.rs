...
fn handle_network_event(event: DroneEvent, active_connections: &Arc<Mutex<Vec<(usize, usize)>>>) {
    match event {
...
fn draw_connections(positions: &[Position], active_connections: &Arc<Mutex<Vec<(usize, usize)>>>) {
