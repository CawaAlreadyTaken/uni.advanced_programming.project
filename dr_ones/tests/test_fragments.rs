...
pub fn fixed_generic_chain_fragment_drop<T: Drone + Send + 'static>() {
    // Client 1 channels
...
pub fn fixed_generic_chain_fragment_ack<T: Drone + Send + 'static>() {
    // Client<1> channels
    let (c_send, c_recv) = unbounded();
    // Server<21> channels
...
fn chain_fragment_drop() {
    fixed_generic_chain_fragment_drop::<Dr_One>();
}

#[test]
fn chain_fragment_ack() {
    fixed_generic_chain_fragment_ack::<Dr_One>();
}
